...
def kfold(u,pra,ite,n,wa,wb):  
    ...
    for train, test in kf.split(dR):
        ...
        B = np.sum(A,axis=1)
        for i in range(lu,lu+lm):
            print(B[i])
        t_u = u
        #クエリを立てる
        shape_y = (1,lu + lm + lta)
        y = sparse.lil_matrix(shape_y)
        ytmp = A[t_u].copy()
        for i in range(lm):
            if(R[t_u,i] != 0):
                y[0,lu + i] = ytmp[0,lu + i]
        
        for i in range(lt):
            if(H[t_u,lr + i]):
                y[0,lu + lm + dT.at[i,'tag_num']] = ytmp[0,lu + lm + dT.at[i,'tag_num']]
        y[0,t_u] = 1
        y = y / np.sum(y)
        f = y.copy()
        old_f = y.copy()
        ...
        for i in range(lrt):
            if(test_df.at[i,'userId'] - 1 == t_u):
                z[test_df.at[i,'movieid']]  = test_df.at[i,'rating']
        for i in range(lm):
            #x2[i] = z[np.argsort(x)[::-1][i]]
            x2[np.argsort(x)[::-1][i]] = z[i]
        x3 = x2[x2 > 0]
        ln = len(x3)
    ...
    print('ave_ndcg: '+ str(round((ndcg_sum/5),5)) )
